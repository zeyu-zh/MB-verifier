...
#include "../libverimb/base64/base64.h"
...
bool VeriTools::initIDS(PMAdaptor ** engine, PatternSet & pattern)
{
	...
	*engine = new ACAdaptor();
    ...
    ((ACAdaptor*)(*engine))->read_nodeHMAC(nodeHMACFilePath);
...
void VeriTools::updateFlowVeri(veriInfo & flow_veri, std::string & pktVeriResult, Packet* p_in)
{
	flow_veri.pktCount += 1;
	//flow_veri.veriRes.veriNum *= getVeriRandom(p_in);
	//flow_veri.veriRes.veriNum += pktVeriResult.veriNum;
	flow_veri.veriRes += std::to_string(getVeriRandom(p_in));
	flow_veri.veriRes += pktVeriResult;
	return;
