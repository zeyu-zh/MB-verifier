...
namespace aho_corasick {
	...
	public:
        size_t                         d_depth;
		ptr                            d_root;
		std::map<CharType, unique_ptr> d_success; // success pointer, multiple patterns, point to state
		...
		void get_all_states(){
			...
			uint8_t* p_key = (uint8_t*)malloc(16);
			...
			for(const auto& a : all_states){
                string_type temp = "";
                //unsigned long d_depth
                temp += std::to_string(a->d_depth);
                //map<CharType, unique_ptr> d_success
                for (auto i = a->d_success.begin(); i != a->d_success.end(); ++i) {
                    temp += i->first;
                    temp += std::to_string(i->second->d_num);
                }

				if(a->d_failure != nullptr)
					temp += std::to_string(a->d_failure->d_num);

				//set<pair<string_type, unsigned>> d_emits
				...
				// calculate???
				//(const uint8_t*)temp.c_str(), temp.length(), p_key, 16, hmac, 16);
                ippsHMAC_Message((uint8_t*)temp.c_str(), temp.length(), p_key, 16, hmac, 16, ippHashAlg_SHA256);
		...
		state_collection get_states() const {
			state_collection result(d_success.size());
			int i = 0;
	...
	public:
		...
		basic_trie(const config& c)
			...
			, d_constructed_failure_states(false) {
                state<char>::all_states.push_back(d_root.get());
            }
        ...
        //         } else {
        //             /*当前节点是某个pattern的结尾*/
        //             char* str = (char*)(ac_queue.front()->get_emits().begin()->first.c_str());
                    
		// 			//pattern str加到suffixP指向的节点的bloom上，为当前节点bloom
		// 			bloom_add(&(ac_queue.front()->d_bloom), str, sizeof(str));
        //         }
        //         ac_queue.pop();
		// 	}
		// }
		...
		void construct_acc()
        {
			...
			while(!ac_queue.empty()) {
                ...
                for(auto& tempstate : ac_queue.front()->get_emits()){
					...
					ippsHMAC_Message((uint8_t*)temp.c_str(), temp.length(), p_key, 16, ac_queue.front()->d_acc, 16, ippHashAlg_SHA256);
                }
