...
void DFC_PrintInfo(DFC_STRUCTURE* dfc){
	...
	ocall_printf("\n");
	ocall_printf("+- [ Direct Filter + Compact Table(DFC) Summary ] -------------------------------------\n");
	ocall_printf("| Patterns: %5d (Case-Insensitive patterns: %d)\n", dfc->numPatterns, nocase_pat_cnt);
	ocall_printf("|   - 1B      : %5d\n", nl1);
	ocall_printf("|   - 2B      : %5d\n", nl2);
	ocall_printf("|   - 3B      : %5d\n", nl3);
	ocall_printf("|   - 4B ~ 7B : %5d\n", nl4);
	ocall_printf("|   - 8B ~    : %5d\n", nl8);
	ocall_printf("|\n");
	ocall_printf("| < Direct Filter Density > \n");
	ocall_printf("|   1. All patterns(DF1)            : %.6f (%4d)\n", (double)nb1/DF_SIZE, nb1);
	ocall_printf("|     1) 1B patterns(DF4)           : %.6f (%4d)\n", (double)nb4/DF_SIZE, nb4);
	ocall_printf("|     2) 2B patterns(DF5)           : %.6f (%4d)\n", (double)nb5/DF_SIZE, nb5);
	ocall_printf("|     4) 4B ~ patterns(DF2)         : %.6f (%4d)\n", (double)nb2/DF_SIZE, nb2);
	ocall_printf("|       (1) 4B ~ 7B patterns(DF8)   : %.6f (%4d)\n", (double)nb8/DF_SIZE, nb8);
	ocall_printf("|          I. 4B ~ 7B patterns(DF9) : %.6f (%4d)\n", (double)nb9/DF_SIZE, nb9);
	ocall_printf("|       (2) 8B ~ patterns(DF3)      : %.6f (%4d)\n", (double)nb3/DF_SIZE, nb3);
	ocall_printf("|\n");
	ocall_printf("| < Compact Table 1 Density >\n");
	ocall_printf("|   - Compact Table 1(CT1) Size   : %u\n", CT1_TABLE_SIZE);
	ocall_printf("|   - Number of non-empty buckets : %d\n", ct1_array_cnt);
	ocall_printf("|   - Total num of PIDs in CT1    : %d\n", ct1_pid_cnt);
	ocall_printf("|   - Number of PIDs per bucket   : %.3f\n", (double)ct1_pid_cnt/ct1_array_cnt);
	ocall_printf("|   - Std Deviation for # of PID  : %.3f\n", ct1_pid_std_dev);
	ocall_printf("|\n");
	ocall_printf("| < Compact Table 2 Density >\n");
	ocall_printf("|   - Compact Table 2(CT2) Size   : %u\n", CT2_TABLE_SIZE);
	ocall_printf("|   - Total # of buckets in CT2   : %d\n", ct2_array_cnt);
	ocall_printf("|   - Avr # of fragments/bucket   : %.3f (none zero)\n", (double)ct2_array_tot_cnt/ct2_array_cnt);
	ocall_printf("|   - Std Dev for # of fragments  : %.3f\n", ct2_array_std_dev);
	ocall_printf("|   - Min/Max for # of fragments  : %u / %u\n", ct2_array_min, ct2_array_max);
	ocall_printf("|\n");
	ocall_printf("|     - Total # of PIDs in CT2    : %d\n", ct2_pid_tot_cnt);
	ocall_printf("|     - Avr # of PIDs per frag    : %.3f (none zero)\n", (double)ct2_pid_tot_cnt/ct2_pid_cnt);
	ocall_printf("|     - Std Dev for # of PID      : %.3f\n", ct2_pid_std_dev);
	ocall_printf("|     - Min/Max for # of PID      : %u / %u\n", ct2_pid_min, ct2_pid_max);
	ocall_printf("|\n");
	ocall_printf("| < Compact Table 4 Density >\n");
	ocall_printf("|   - Compact Table 4(CT4) Size   : %u\n", CT4_TABLE_SIZE);
	ocall_printf("|   - Total # of buckets in CT4   : %d\n", ct4_array_cnt);
	ocall_printf("|   - Avr # of fragments/bucket   : %.3f (collision rate: %.5f (%d/%d))\n", 
	...
	ocall_printf("|\n");
	ocall_printf("|     - Total # of PIDs in CT4    : %d\n", ct4_pid_tot_cnt);
	ocall_printf("|     - Avr # of PIDs per frag    : %.3f (none zero)\n", (double)ct4_pid_tot_cnt/ct4_pid_cnt);
	ocall_printf("|     - Std Dev for # of PID      : %.3f\n", ct4_pid_std_dev);
	ocall_printf("|     - Min/Max for # of PID      : %u / %u\n", ct4_pid_min, ct4_pid_max);
	ocall_printf("|\n");
	ocall_printf("| < Compact Table 8 Density >\n");
	ocall_printf("|   - Compact Table 8(CT8) Size   : %u\n", CT8_TABLE_SIZE);
	ocall_printf("|   - Total # of buckets in CT8   : %d\n", ct8_array_cnt);
	ocall_printf("|   - Avr # of fragments/bucket   : %.3f (collision rate: %.5f (%d/%d))\n", 
	...
	ocall_printf("|\n");
	ocall_printf("|     - Total # of PIDs in CT8    : %d\n", ct8_pid_tot_cnt);
...
/* ####################################################################################### */
	...
	for(plist = dfc->dfcPatterns; plist != NULL; plist = plist->next){
        if(dfc->dfcMatchList[plist->iid] != NULL){
			...
			ocall_printf("Internal ID ERROR : %u\n", plist->iid);
		}
			...
			if(dfc->CompactTable1[temp[0]].cnt == 0){
				...
				if(k == dfc->CompactTable1[temp[0]].cnt){
					...
					if(dfc->CompactTable1[temp[0]].cnt >= CT_TYPE1_PID_CNT_MAX)
						ocall_printf("Too many PIDs in CT1. You should expand the size.\n");
				}
			}
		...
		for(n=0; n < dfc->CompactTable2[i].cnt; n++){
			...
			if(dfc->CompactTable2[i].array[n].cnt >= RECURSIVE_BOUNDARY){
				...
				if(!dfc->CompactTable2[i].array[n].DirectFilter || !dfc->CompactTable2[i].array[n].CompactTable){
					...
					ocall_printf("Failed to allocate memory for recursive things.\n");
					return -1;
				}
...
static void * DFC_REALLOC(void *p, uint16_t n, dfcDataType type, dfcMemoryType type2 )
{
	switch(type){
		...
		case DFC_CT_Type_2_8B_Array:
			...
			return p;
		default:
			ocall_printf("ERROR! Data Type is not correct!\n");
			break;
	}
	return NULL;
}
    ...
    if (p != NULL)
    {
        switch (type)
        {
            ...
            case DFC_MEMORY_TYPE__NONE:
                break;
            default:
                ocall_printf("%s(%d) Invalid memory type\n", __FILE__, __LINE__);
                break;
        }
        dfc_total_memory += n;
    }
